...
namespace Restaurant.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var check = SignInManager.UserManager.CheckPassword(user, model.Password);
            if (user != null && check)
            {
                if (!CheckConfirmEmail(user.Email))
                {
                    SendConfirmEmail(user.Id, model.Email);
            ...
            switch (result)
            {
                ...
                default:
                    ModelState.AddModelError("", "ایمیل یا رمز عبور نادرست است!");
                    return View(model);
            }
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model, [Bind(Include = "PhoneNumber")] string PhoneNumber)
        {
            if (ModelState.IsValid && PhoneNumber != null && PhoneNumber != "")
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, PhoneNumber = PhoneNumber };
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    GetManager().AddToRoles(user.Id, "User");
                    if (user != null)
                    {
                        if (!CheckConfirmEmail(user.Email))
                        {
        ...
        public async Task<ActionResult> ConfirmEmail(string email, string code)
        {
            ...
            var user = SignInManager.UserManager.FindByEmail(email);
        ...
        public bool CheckConfirmEmail(string Email)
        {
            if (Email != null)
            {
